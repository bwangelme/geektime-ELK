...
def search(query, printHighlight):
    print(query)
    url = "http://192.168.0.102:9200/%s/_search" % indexName
    ...
    for idx, hit in enumerate(hits["hits"]):
        ...
        print("---------------------------------------------------------------")
        if printHighlight:
            if (("highlight" in hit.keys()) and (
                    "title" in hit["highlight"].keys())):
                hl = ";".join(hit["highlight"]["title"]).replace("<em>",
                                                                 "\033[1;31;40m").replace(
                    "</em>", "\033[0m")
                print("title: \033[0;32;40m%d hit(s)\033[0m \r\n%s\r\n--" % (
                    len(hit["highlight"]["title"]), hl))
            if (("highlight" in hit) and ("overview" in hit["highlight"])):
                hl = ";".join(hit["highlight"]["overview"]).replace("<em>",
                                                                    "\033[1;31;40m").replace(
                    "</em>", "\033[0m")
